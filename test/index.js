...
function singleArgFn(arg) {
	return arg;
}

function multipleArgFn(arg1,arg2) {
	return {arg1:arg1,arg2:arg2};
}

const singleArg = nanomemoize(singleArgFn);

const multipleArg = nanomemoize(multipleArgFn);

const varArg = nanomemoize(function() { return [].slice.call(arguments); });

const callTimeout = nanomemoize(function(a,b,cb) { var result = a + b; cb(result); return result; },{maxArgs:2,callTimeout:0});
...
describe("Test",function() {
	...
	it("single primitive number arg cached",function() {
		const value = 1;
		const result = singleArg(value);
		const keyvalues = singleArg.keyValues().primitives;
	...
	it("multiple arg object cached",function() {
		const arg1 = {arg:1};
		const arg2 = {arg:2};
		const result = multipleArg(arg1,arg2);
